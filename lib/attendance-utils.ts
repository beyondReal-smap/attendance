...
export function isLeaveType(type: AttendanceType): boolean {
  return ['연차', '체휴', '오전반차', '오후반차', '오전반반차A', '오전반반차B', '오후반반차A', '오후반반차B'].includes(type);
}
...
export function isAnnualLeaveType(type: AttendanceType): boolean {
  return ['연차', '오전반차', '오후반차', '오전반반차A', '오전반반차B', '오후반반차A', '오후반반차B'].includes(type);
}
